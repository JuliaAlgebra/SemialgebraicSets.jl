...
@testset "Basic semialgebraic set" begin
    ...
    @testset "Basic" begin
        ...
        @test sprint(show, S) ==
              "{ (x, y) | x - y = 0, x^2 - y = 0, x^2*y - 1//1 ≥ 0, x + y - 1//1 ≥ 0 }"
        ...
        @test S ===
              MultivariatePolynomials.changecoefficienttype(S, Rational{BigInt})
        @test S.V === MultivariatePolynomials.changecoefficienttype(
            S.V,
            Rational{BigInt},
        )
        @test S.V.I === convert(typeof(S.V.I), S.V.I)
        @test BasicSemialgebraicSet{Int,polynomialtype(x, Int)}() isa
              BasicSemialgebraicSet{
            Rational{BigInt},
            polynomialtype(x, Rational{BigInt}),
        }
...
"""
        ...
        @test S.V ===
              MultivariatePolynomials.changecoefficienttype(S.V, Float64)
    end
